...
def add_supercontour(
    dwg: svgwrite.Drawing, svg_path: str, stroke_color: str = "black", stroke_width: float = 2.0
):
...
def image_to_svg(
    ...
    rgb = load_image_rgba_handled(
        input_path, bg_color, alpha_mode=alpha_mode, alpha_threshold=alpha_threshold
    )
    h, w = rgb.shape[:2]
    ...
    for idx, color in enumerate(palette_arr):
        ...
        paths = build_compound_paths(
            contours, hierarchy, min_area, min_hole_area, epsilon, smooth, bezier, precision
        )
        if not paths:
